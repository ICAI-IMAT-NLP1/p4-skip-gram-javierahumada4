...
class SkipGramNeg(nn.Module):
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        # Normalize the noise distribution so that it sums to 1
        ...
        noise_vectors: torch.Tensor = self.in_embed(noise_words)
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(self, input_vectors: torch.Tensor, output_vectors: torch.Tensor,
        ...
        pos_dot_product = torch.bmm(input_vectors.unsqueeze(1), output_vectors.unsqueeze(2)).squeeze() 
        out_loss = F.logsigmoid(pos_dot_product)

        # Compute log-sigmoid loss for incorrect classifications
        neg_dot_product = torch.bmm(noise_vectors, input_vectors.unsqueeze(2)).squeeze()
        noise_loss = F.logsigmoid(-neg_dot_product).sum(dim=1)
